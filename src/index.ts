...
export default function ImagePresetsPlugin(presets?: ImagePresets, options?: Options) {
  let api: ImageApi;
  let config: Config;
  ...
  return {
    ...
    async configResolved({ base, command, root, build: { assetsDir } }: ResolvedConfig) {
      ...
      if (config.isBuild) await fs.mkdir(config.cacheDir, { recursive: true });
    },
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use(async (req: any, res: any, next: any) => {
        if (req.url?.startsWith(VIRTUAL_ID)) {
          ...
          if (!image) {
            console.error(
              `vite-image-presets cannot find image with id "${id}" this is likely an internal error`,
            );
            res.statusCode = 404;
            return res.end();
          }
    ...
    async generateBundle(this: {
      emitFile: (file: { type: string; fileName: string; source: Buffer | string }) => void;
    }) {
