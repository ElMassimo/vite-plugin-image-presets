  ...
  return {
    ...
    async writeImages(outDir: string) {
      ...
      const images = await Promise.all(
        generatedImages.map(async (imagePromise) => {
          const image = await imagePromise;
          await fs.writeFile(join(outDir, image.fileName), image.source);
          return image;
        }),
      );
      await this.purgeCache(images);
    },
    async purgeCache(assets: OutputAsset[]) {
      ...
      await Promise.all(
        unusedFiles.map((file) => fs.rm(resolve(config.cacheDir, file), { force: true })),
      );
    },
    async resolveImage(
      filename: string,
      params: Record<string, any>,
    ): Promise<undefined | string | ImageAttrs[]> {
      const { [config.urlParam]: presetName, src, srcset, ...otherParams } = params;
      const preset = config.presets[presetName];

      debug.load("%O %s", params, filename);

      if (!preset) throw new Error(`vite-image-presets: Unknown image preset '${presetName}'`);
      ...
      if (src !== undefined) return lastSrc;

      if (srcset !== undefined) {
        const attrs = imagesAttrs[srcset === "" ? imagesAttrs.length - 1 : Number(srcset)];
        if (!attrs)
          throw new Error(
            `The '${presetName}' image preset did not provide any source matching the provided index: ${srcset}.\nURL: ${filename}?${new URLSearchParams(params)}`,
          );
        return attrs.srcset;
      }
      ...
      if (preset.inferDimensions) {
        ...
        const {
          info: { width, height },
        } = await lastSharp.toBuffer({ resolveWithObject: true });
